...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        def find_place(range):
            assert range>=0
            place=self.place
            while(range>0 and place is not None):
                if(place is beehive):
                    break
                place=place.entrance
                range-=1
            return place
        farthest_place_plus1=find_place(self.max_range+1)
        nearest_place=find_place(self.min_range)
        cur_place=nearest_place
        while(cur_place is not farthest_place_plus1):
            if rANTdom_else_none(cur_place.bees)!=None:
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        # BEGIN Problem 5
        "*** YOUR CODE HERE ***"
        def make_damage(amount):
            remening_bees=[]
            for bee in self.place.bees:
                if(bee.armor>amount):
                    remening_bees.append(bee)
            for bee in self.place.bees.copy():
                bee.reduce_armor(amount)
            self.place.bees=remening_bees
        make_damage(amount)
        if(self.armor<=amount):
            make_damage(self.damage)
